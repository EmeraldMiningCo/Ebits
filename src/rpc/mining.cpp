...
// NOTE: Unlike wallet RPC (which use EBITS values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EBITS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EBITS is downloading blocks...");
...
static UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateEBITSs(fGenerate, nGenProcLimit, Params(), *g_connman);
