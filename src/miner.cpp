...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(CWallet *wallet, const CScript &scriptPubKeyIn, bool fProofOfStake, bool fMineWitnessTx)
{
    ...
    else
    {
		...
		CEBITSAddress address2(address1);
...
void static EBITSMiner(const CChainParams& chainparams, CConnman& connman, CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (true) {
        try {
            ...
            LogPrintf("EBITSMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            if (fProofOfStake)
            {
                ...
                if (!SignBlock(*pblock, *pwallet)) {
                    LogPrintf("EBITSMiner(): Signing new block failed \n");
        ...
        {
            LogPrintf("EBITSMiner -- terminated\n");
            throw;
        }
...
void GenerateEBITSs(bool fGenerate,
                  int nThreads,
                  ...
                  CConnman &connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EBITSMiner, boost::cref(chainparams), boost::ref(connman), GetWallets().front(), false));
}
...
void ThreadStakeMinter(const CChainParams &chainparams, CConnman &connman, CWallet *pwallet)
{
    ...
    try {
        EBITSMiner(chainparams, connman, pwallet, true);
        boost::this_thread::interruption_point();
